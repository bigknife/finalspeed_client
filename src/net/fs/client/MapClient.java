...
public class MapClient implements Trafficlistener {
	...
	MapClient(ClientUI ui) throws Exception {
		...
		/*
		 * try { route_tcp = new Route(null, routePort, Route.mode_client, true,
		 * tcpEnvSuccess); } catch (Exception e1) { //e1.printStackTrace();
		 * throw e1; }
		 */
		try {
			// route_udp = new Route(null, routePort, Route.mode_client, false,
			// tcpEnvSuccess);
			route_udp = new Route(null, routePort);
		} catch (Exception e1) {
			// e1.printStackTrace();
			throw e1;
		}
	...
	// public void setMapServer(String serverAddress, int serverPort, int
	// remotePort, String passwordMd5, String password_proxy_Md5, boolean
	// direct_cn, boolean tcp, String password) {
	public void setMapServer(String serverAddress, int serverPort, int remotePort, String passwordMd5,
			String password_proxy_Md5, boolean direct_cn, String password) {
	...
	void setFireWallRule(String serverAddress, int serverPort) {
		...
		try {
			...
			if (systemName.contains("mac os")) {
				...
				String cmd2 = "iptables -t filter -A OUTPUT -d " + ip + " -p tcp --dport " + serverPort
						+ " -j DROP -m comment --comment tcptun_fs ";
				runCommand(cmd2);
			} else if (systemName.contains("windows")) {
				try {
					if (systemName.contains("xp") || systemName.contains("2003")) {
						String cmd_add1 = "ipseccmd -w REG -p \"tcptun_fs\" -r \"Block TCP/" + serverPort
								+ "\" -f 0/255.255.255.255=" + ip + "/255.255.255.255:" + serverPort
								+ ":tcp -n BLOCK -x ";
						final Process p2 = Runtime.getRuntime().exec(cmd_add1, null);
						p2.waitFor();
					} else {
						String cmd_add1 = "netsh advfirewall firewall add rule name=tcptun_fs protocol=TCP dir=out remoteport="
								+ serverPort + " remoteip=" + ip + " action=block ";
						final Process p2 = Runtime.getRuntime().exec(cmd_add1, null);
						p2.waitFor();
						String cmd_add2 = "netsh advfirewall firewall add rule name=tcptun_fs protocol=TCP dir=in remoteport="
								+ serverPort + " remoteip=" + ip + " action=block ";
