...
public class Sender {
	...
	void sendNata(byte[] data, int offset, int length) throws ConnectException, InterruptedException {
		...
		if (!closed) {
			if (!streamClosed) {
				DataMessage me = new DataMessage(sequence, data, 0, (short) length, conn.connectId,
						conn.route.localclientId);
	...
	void sendDataMessage(DataMessage me, boolean resend, boolean twice, boolean block) {
		synchronized (conn.clientControl.getSynlock()) {
			...
			try {
				...
				TrafficEvent event = new TrafficEvent("", ran.nextLong(), me.getData().length,
						TrafficEvent.type_uploadTraffic);
	...
	void sendALMessage(ArrayList ackList) {
		...
		AckListMessage alm = new AckListMessage(conn.connetionId, ackList, conn.receiver.lastRead,
				conn.clientControl.sendRecordTable_remote, currentTimeId, conn.connectId, conn.route.localclientId);
