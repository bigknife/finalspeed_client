...
public class AckListMessage extends Message {
	...
	public AckListMessage(long connId, ArrayList ackList, int lastRead, HashMap<Integer, SendRecord> sendRecordTable,
		...
		ByteShortConvert.toByteArray((short) ackList.size(), dpData, 8 + 8); // add:
																				// service
																				// type
		for (int i = 0; i < ackList.size(); i++) {
			int sequence = (Integer) ackList.get(i);
			ByteIntConvert.toByteArray(sequence, dpData, 10 + 4 * i + 8); // add:
		}
