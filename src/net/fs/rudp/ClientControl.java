...
public class ClientControl {
	...
	public void onReceivePacket(DatagramPacket dp) {
		...
		if (sType == net.fs.rudp.message.MessageType.sType_PingMessage) {
			...
			//	pingDelay = (int) (System.currentTimeMillis() - t);
				...
				// String protocal="";
				/*
				 * if(route.isUseTcpTun()){ protocal="tcp";; }else {
				 * protocal="udp"; }
				 */
				// MLog.println(" receive_ping222: "+pm.getPingId()+" "+new
				// Date());
	...
	public void sendPingMessage() {
		...
		PingMessage lm = new PingMessage(0, route.localclientId, pingid, Route.localDownloadSpeed,
				Route.localUploadSpeed);
		...
		if (sended > 10 * 1024) {
			...
			if (usedTime < needTime) {
				...
				if (sleepTime > 0) {
					...
					// #MLog.println("sssssssssss
					// "+(trueSleep_All-needSleep_All)/(1000*1000));
				}
				//// #MLog.println("sleepb "+sleepTime+" l "+sended+" s "+s+" n
				//// "+n+" tt "+(moreTime));
			}
			sended = 0;
		}
