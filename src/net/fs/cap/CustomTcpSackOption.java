...
public final class CustomTcpSackOption implements TcpOption {
	 ...
	 */
	public static CustomTcpSackOption newInstance(byte[] rawData, int offset, int length)
			throws IllegalRawDataException {
	...
	private CustomTcpSackOption(byte[] rawData, int offset, int length) throws IllegalRawDataException {
		...
		if (rawData[offset] != kind.value()) {
			...
			sb.append("The kind must be: ").append(kind.valueAsString()).append(" rawData: ")
					.append(ByteArrays.toHexString(rawData, " ")).append(", offset: ").append(offset)
		...
		if (length < lengthFieldAsInt) {
			StringBuilder sb = new StringBuilder(100);
			sb.append("rawData is too short. length field: ").append(lengthFieldAsInt).append(", rawData: ")
					.append(ByteArrays.toHexString(rawData, " ")).append(", offset: ").append(offset)
					.append(", length: ").append(length);
			throw new IllegalRawDataException(sb.toString());
		}
		...
		for (int i = 2; i < lengthFieldAsInt; i += INT_SIZE_IN_BYTES * 2) {
			sacks.add(new Sack(ByteArrays.getInt(rawData, i + offset),
	...
	public byte[] getRawData() {
		...
		int offset = 2;
		for (Sack sack : sacks) {
			System.arraycopy(ByteArrays.toByteArray(sack.leftEdge), 0, rawData, offset, INT_SIZE_IN_BYTES);
			System.arraycopy(ByteArrays.toByteArray(sack.rightEdge), 0, rawData, offset + INT_SIZE_IN_BYTES,
					INT_SIZE_IN_BYTES);
	...
	public String toString() {
		...
		for (Sack sack : sacks) {
			sb.append(" [LE: ").append(sack.getLeftEdgeAsLong()).append(" RE: ").append(sack.getRightEdgeAsLong())
					.append("]");
		}
		return sb.toString();
	}
	 ...
	 * @since pcap4j 1.2.0
	 */
	public static final class Builder implements LengthBuilder<CustomTcpSackOption> {
