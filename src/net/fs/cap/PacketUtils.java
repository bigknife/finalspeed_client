...
public class PacketUtils {
	...
	public static Packet buildIpV4(MacAddress srcAddress_mac, MacAddress dstAddrress_mac,
		...
		etherBuilder.dstAddr(dstAddrress_mac).srcAddr(srcAddress_mac).type(etherType).payloadBuilder(builder)
				.paddingAtBuild(true);
	...
	static Packet createDataPacket(MacAddress srcAddress_mac, MacAddress dstAddrress_mac, Inet4Address srcAddress,
			short srcPort, Inet4Address dstAddress, short dstPort, int sequence, int ack, byte[] data, short ident) {
		Packet p = null;
	...
	static Packet createAck(MacAddress srcAddress_mac, MacAddress dstAddrress_mac, Inet4Address srcAddress,
			short srcPort, Inet4Address dstAddress, short dstPort, int ack_sequence, int sequence, short ident) {

		TcpPacket.Builder builder_tcp = new TcpPacket.Builder();
		// builder_tcp.payloadBuilder(new UnknownPacket.Builder().rawData(new
		// byte[0]));
		builder_tcp.correctChecksumAtBuild(true);
		builder_tcp.correctLengthAtBuild(true);
		builder_tcp.paddingAtBuild(true);
		...
		// System.out.println("自定义确认 "+" identification "+identification+"
		// ack_sequence "+ack_sequence+" # "+tcpPacket.getHeader());
		return p;

	}

	static Packet createSyncAck(MacAddress srcAddress_mac, MacAddress dstAddrress_mac, Inet4Address srcAddress,
			short srcPort, Inet4Address dstAddress, short dstPort, int ack_sequence, int sequence, short ident) {
	...
	static Packet createSync(MacAddress srcAddress_mac, MacAddress dstAddrress_mac, Inet4Address srcAddress,
			short srcPort, Inet4Address dstAddress, short dstPort, int sequence, short ident) {
		TcpPacket.Builder builder_tcp = new TcpPacket.Builder();
		// builder_tcp.payloadBuilder(new UnknownPacket.Builder().rawData(new
		// byte[0]));
		builder_tcp.correctChecksumAtBuild(true);
